...
(function (global, factory) {
      ...
      forEachEvent(events, function (event, namespace) {
        ...
        handlers[event] =
          ...
          eventHandlers.filter(function (handler) {
            return handler._key
              ? handler._key !== key
              : key || handler._namespace !== namespace;
          });
      });
    }
  ...
  function Slide$1(Splide2, index, slideIndex, slide) {
    ...
    var focusableNodes =
      options.focusableNodes && queryAll(slide, options.focusableNodes);
    var destroyed;
    ...
    function computeCloneCount() {
      ...
      if (!Splide2.is(LOOP)) {
        ...
        var baseCount =
          ...
          options.perPage;
        clones2 =
          baseCount * (options.drag ? (options.flickMaxPages || 1) + 1 : 2);
      }
  ...
  function Controller(Splide2, Components2, options) {
    ...
    function scroll(destination, useIndex, snap, duration, callback) {
      ...
      Components2.Scroll.scroll(
        useIndex || snap ? Move.toPosition(dest, true) : destination,
        duration,
        function () {
      ...
      if (isEnough()) {
        ...
        if (dest < 0 || dest > end) {
          ...
          if (!incremental && dest !== from) {
            dest = perMove
              ? dest
              : toIndex(toPage(from) + (dest < from ? -1 : 1));
          }
        }
      } else {
        dest = -1;
    ...
    function loop(index) {
      if (isLoop) {
        return isEnough()
          ? (index % slideCount) + (index < 0 ? slideCount : 0)
          : -1;
      }
    ...
    function toPage(index) {
      if (!hasFocus()) {
        index = between(index, slideCount - perPage, slideCount - 1)
          ? slideCount - 1
          : index;
  ...
  function Arrows(Splide2, Components2, options) {
    ...
    function createArrows() {
      ...
      before(
        wrapper,
        child((options.arrows === "slider" && Elements.slider) || Splide2.root)
      );
    }
  ...
  function Cover(Splide2, Components2, options) {
    ...
    function toggle(cover, img, Slide) {
      Slide.style(
        "background",
        cover ? 'center/cover no-repeat url("' + img.src + '")' : "",
        true
      );
    ...
    function onPointerMove(e) {
      ...
      if (e.cancelable) {
        ...
        if (dragging) {
          ...
          dragging =
            abs(diff) > (isTouchEvent(e) ? thresholds.touch : thresholds.mouse);
  ...
  function LazyLoad(Splide2, Components2, options) {
    ...
    function mount() {
      if (options.lazyLoad) {
        ...
        if (!isSequential) {
          on(
            [EVENT_MOUNTED, EVENT_REFRESH, EVENT_MOVED, EVENT_SCROLLED],
            observe
          );
        }
      }
    ...
    function load(data) {
      ...
      ["srcset", "src"].forEach(function (name) {
        if (data[name]) {
          ...
          removeAttribute(
            _img,
            name === "src" ? SRC_DATA_ATTRIBUTE : SRCSET_DATA_ATTRIBUTE
          );
        }
      });
    }
  ...
  function Pagination(Splide2, Components2, options) {
    ...
    function createPagination() {
      ...
      var classes = options.classes,
        ...
        perPage = options.perPage;
      var parent =
        (options.pagination === "slider" && Elements.slider) || Elements.root;
  ...
  function Sync(Splide2, Components2, options) {
    ...
    function update() {
      setAttribute(
        list,
        ARIA_ORIENTATION,
        options.direction !== TTB ? "horizontal" : null
      );
    }
  ...
  var _Splide = /*#__PURE__*/ (function () {
    ...
    _proto.mount = function mount(Extensions, Transition) {
      ...
      this._Transition =
        Transition || this._Transition || (this.is(FADE) ? Fade : Slide);
